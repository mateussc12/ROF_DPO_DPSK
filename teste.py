...
def debug(plot_enable, Tx_bin_wave, V_1, V_2, Tx_bin_wave_DPSK_encoded, NPPS, t, E_out, E_out_noise, Esync_filtered,
    ...
    # Faz o downsample no sinal
    downsampled_Esync = signal.resample(normalized_Esync_filtered_cutted, int(np.size(normalized_Esync_filtered_cutted) / NPPS))
    ...
    if eye_enable:
        ...
        eye_signal, eye_t = eye_diagram.eye_diagram(downsampled_Esync, Ts, NPPS)
    ...
    if dig_decision == 1:
        Rx_bin_wave = digitalize.digitalize(downsampled_Esync, NPPS, Num_Simb, treshold=0)
    elif dig_decision == 2:
        Rx_bin_wave = digitalize.digitalize(downsampled_Esync, NPPS, Num_Simb, treshold='mean')
    elif dig_decision == 3:
        eye_treshold = digitalize.Eye_treshold(normalized_Esync_filtered_cutted)
        Rx_bin_wave = digitalize.digitalize(downsampled_Esync, NPPS, N_Simb, treshold=eye_treshold)
    ...
    if plot_enable:
        ...
        if dig_decision == 3:
            ...
            plt.plot(downsampled_Esync)
            plt.plot(np.zeros(np.size(downsampled_Esync)), label='0')
            mean = np.mean(downsampled_Esync)
            plt.plot(np.repeat(mean, np.size(downsampled_Esync)), label=f'mean = {np.round(mean, 5)}', linewidth=3)
            plt.plot(np.repeat(eye_treshold, np.size(downsampled_Esync)),
                     label=f'eye_treshold = {np.round(eye_treshold, 5)}')
